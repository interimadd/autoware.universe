"""
take版のRelayノードを1つ立ち上げ、そのノードで全てのtopicを受信してpublishするLaunchファイル
受信するtopicの数を引数で指定可能

使い方：
ros2 launch autoware_default_adapi_poc take_relay_one_node.launch.py topic_num:=10
"""
...
def generate_take_relay_node(topic_num: int) -> Node:
    return Node(
        package='autoware_default_adapi_poc',
        executable='take_relay_node',
        parameters=[
...
def generate_launch_description():
    ...
    def launch_setup(context, *args, **kwargs):
        ...
        nodes = [
            generate_take_relay_node(topic_num)
        ]
        nodes += [
