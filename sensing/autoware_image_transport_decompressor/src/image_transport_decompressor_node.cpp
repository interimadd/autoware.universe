...
{
  const auto decompressed_image = decompress_image(*input_compressed_image_msg, encoding_);
  if (!decompressed_image.has_value()) {
    RCLCPP_ERROR(get_logger(), "%s", decompressed_image.error().c_str());
    return;
  }

  // Publish message to user callback
  auto image_ptr = std::make_unique<sensor_msgs::msg::Image>(decompressed_image.value());
  raw_image_pub_->publish(std::move(image_ptr));
}
}  // namespace autoware::image_transport_decompressor

#include <rclcpp_components/register_node_macro.hpp>
RCLCPP_COMPONENTS_REGISTER_NODE(autoware::image_transport_decompressor::ImageTransportDecompressor)
